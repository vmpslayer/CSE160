...
implementation{
    ...
    command void LinkStateRouting.receiveHandler(pack myMsg){
        ...
        memcpy(linkTable[flood_pack.floodSource].neighbors, &flood_pack.payload, PACKET_MAX_PAYLOAD_SIZE);
        for(i = 1; i < MAX_NEIGHBORS; i++){
            if(nodeTable[i].address != 0){
                linkTable[flood_pack.floodSource].neighbors[i] = 1;
            }
            for(j = 0; j < MAX_NEIGHBORS; j++){
                if(linkTable[i].neighbors[j] != 0 && linkTable[i].neighbors[j] != 1){
                    linkTable[i].neighbors[j] = 1;
                }
                else if(linkTable[i].neighbors[j] == 0){
                    linkTable[i].neighbors[j] = 0;
                }
            }
        }
